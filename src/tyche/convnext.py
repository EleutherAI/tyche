...
def load_convnext_checkpoint(
    ...
    config = ConvNextV2Config.from_pretrained(checkpoint_path, device_map=device)

    # Create model with config
    model = ConvNextV2ForImageClassification.from_pretrained(
        checkpoint_path, config=config, torch_dtype=torch.float16, device_map=device
    )
...
def load_cifar10_splits(
    ...
    clean_split = train_split["train"].add_column(
        "is_poison", [False] * len(train_split["train"])
    )
    ...
    clean_split = clean_split.map(
        preprocess, batched=True, remove_columns=clean_split.column_names
    )
    poison_split = poison_split.map(
        preprocess, batched=True, remove_columns=poison_split.column_names
    )
    val_ds = val_ds.map(preprocess, batched=True, remove_columns=val_ds.column_names)
...
def load_cifar10_splits_dict(
    ...
    return {
        "clean": torch.tensor(clean_split["pixel_values"]).to(
            device=device, dtype=torch.float16
        ),
        "clean_labels": torch.tensor(clean_split["label"]).to(device),
        "poison": torch.tensor(poison_split["pixel_values"]).to(
            device=device, dtype=torch.float16
        ),
        "poison_labels": torch.tensor(poison_split["label"]).to(device),
        "val": torch.tensor(val_ds["pixel_values"]).to(
            device=device, dtype=torch.float16
        ),
        "val_labels": torch.tensor(val_ds["label"]).to(device),
    }


def get_convnext_logits(
...
def load_convnext_adam_vectors(model, run_name, step):
    with open(
        f"{BASIN_VOLUME_DIR}/runs/{run_name}/checkpoint-{step}/optimizer.pt", "rb"
    ) as f:
        states = torch.load(f)
    with open(f"{BASIN_VOLUME_DIR}/data/convnext_param_mapping.json", "r") as f:
        param_mapping = json.load(f)
    all_groups_mapping = {
        int(j): param_mapping[i][j] for i in param_mapping for j in param_mapping[i]
    }
    adam_param_names = [all_groups_mapping[i] for i in range(len(states["state"]))]
    adam1_dict = {
        adam_param_names[i]: states["state"][i]["exp_avg"]
        for i in range(len(states["state"]))
    }
    adam2_dict = {
        adam_param_names[i]: states["state"][i]["exp_avg_sq"]
        for i in range(len(states["state"]))
    }
